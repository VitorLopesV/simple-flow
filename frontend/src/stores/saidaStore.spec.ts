import { flushPromises } from '@vue/test-utils'
import { createPinia, setActivePinia } from 'pinia'
import { beforeEach, describe, expect, it, vi } from 'vitest'

import { saidaService } from '@/services/saidaService'
import { usePeriodoStore } from '@/stores/periodoStore'
import { useSaidaStore } from '@/stores/saidaStore'
import type { Saida, SaidaPayload, SaidaResumo } from '@/types/saida'

// Só o service é substituído: `compararSaidasPorVencimento` continua real.
vi.mock('@/services/saidaService', async (importarOriginal) => {
  const original = await importarOriginal<typeof import('@/services/saidaService')>()
  return {
    ...original,
    saidaService: {
      listar: vi.fn(),
      resumo: vi.fn(),
      criar: vi.fn(),
      atualizar: vi.fn(),
      remover: vi.fn(),
    },
  }
})

const servico = vi.mocked(saidaService)

function saida(id: string, sobrescritas: Partial<Saida> = {}): Saida {
  return {
    id,
    descricao: `Saída ${id}`,
    valor: 100,
    data: '2026-08-10',
    categoriaId: 'cat_1',
    tipo: 'CONTA',
    status: 'PENDENTE',
    vencimento: null,
    pagoEm: null,
    formaPagamento: 'PIX',
    cartaoId: null,
    recorrente: false,
    criadoEm: '2026-01-01T00:00:00.000Z',
    atualizadoEm: '2026-01-02T00:00:00.000Z',
    ...sobrescritas,
  }
}

function resumo(sobrescritas: Partial<SaidaResumo> = {}): SaidaResumo {
  return {
    total: 1000,
    quantidade: 2,
    media: 500,
    totalPago: 600,
    totalPendente: 400,
    totalMesAnterior: 800,
    porCategoria: [],
    ...sobrescritas,
  }
}

const PAYLOAD: SaidaPayload = {
  descricao: 'Aluguel',
  valor: 1900,
  data: '2026-08-10',
  categoriaId: 'cat_1',
  tipo: 'CONTA',
  status: 'PENDENTE',
  formaPagamento: 'PIX',
  recorrente: false,
}

const ITENS = [saida('s1'), saida('s2')]

/** Última chamada de `listar`, para conferir o filtro montado pelo store. */
function ultimoFiltro() {
  return servico.listar.mock.calls.at(-1)![0]
}

beforeEach(() => {
  setActivePinia(createPinia())
  usePeriodoStore().definir({ mes: 8, ano: 2026 })

  vi.resetAllMocks()
  servico.listar.mockImplementation(async (filtro) => ({
    items: ITENS,
    page: filtro.page,
    pageSize: filtro.pageSize,
    total: 20,
    totalPages: 3,
  }))
  servico.resumo.mockResolvedValue(resumo())
  servico.criar.mockResolvedValue(saida('novo'))
  servico.atualizar.mockResolvedValue(saida('s1'))
  servico.remover.mockResolvedValue(undefined)
})

describe('estado inicial', () => {
  it('começa vazio, na página 1 e sem filtros', () => {
    const store = useSaidaStore()

    expect(store.itens).toEqual([])
    expect(store.resumo).toBeNull()
    expect(store.page).toBe(1)
    expect(store.pageSize).toBe(8)
    expect(store.totalPages).toBe(1)
    expect(store.categoriaId).toBeNull()
    expect(store.status).toBeNull()
    expect(store.busca).toBe('')
  })
})

describe('carregar', () => {
  it('monta o filtro com período, categoria, status, busca, página e pageSize', async () => {
    usePeriodoStore().definir({ mes: 3, ano: 2025 })
    const store = useSaidaStore()
    store.categoriaId = 'cat_9'
    store.status = 'PAGO'
    store.busca = 'aluguel'
    store.page = 2

    await store.carregar()

    expect(servico.listar).toHaveBeenCalledWith({
      periodo: { mes: 3, ano: 2025 },
      categoriaId: 'cat_9',
      status: 'PAGO',
      busca: 'aluguel',
      page: 2,
      pageSize: 8,
    })
    expect(servico.resumo).toHaveBeenCalledWith({ mes: 3, ano: 2025 })
  })

  it('preenche itens, paginação e resumo', async () => {
    const store = useSaidaStore()

    await store.carregar()

    expect(store.itens).toEqual(ITENS)
    expect(store.total).toBe(20)
    expect(store.totalPages).toBe(3)
    expect(store.page).toBe(1)
    expect(store.resumo).toEqual(resumo())
    expect(store.erro).toBeNull()
    expect(store.loading).toBe(false)
  })

  it('adota a página normalizada devolvida pelo service', async () => {
    servico.listar.mockResolvedValue({ items: ITENS, page: 3, pageSize: 8, total: 20, totalPages: 3 })
    const store = useSaidaStore()
    store.page = 99

    await store.carregar()

    expect(store.page).toBe(3)
  })

  it('mantém loading verdadeiro durante a chamada', async () => {
    let concluir!: () => void
    servico.listar.mockReturnValue(
      new Promise((resolve) => {
        concluir = () => resolve({ items: ITENS, page: 1, pageSize: 8, total: 2, totalPages: 1 })
      }),
    )
    const store = useSaidaStore()

    const promessa = store.carregar()
    expect(store.loading).toBe(true)
    concluir()
    await promessa

    expect(store.loading).toBe(false)
  })

  it('falha: preenche erro, zera itens e desliga o loading', async () => {
    const store = useSaidaStore()
    await store.carregar()
    expect(store.itens).toHaveLength(2)
    servico.listar.mockRejectedValue(new Error('Servidor fora do ar'))

    await store.carregar()

    expect(store.erro).toBe('Servidor fora do ar')
    expect(store.itens).toEqual([])
    expect(store.loading).toBe(false)
  })

  it('falha no resumo também é tratada, com o texto padrão', async () => {
    servico.resumo.mockRejectedValue('quebrou')
    const store = useSaidaStore()

    await store.carregar()

    expect(store.erro).toBe('Não foi possível carregar as saídas.')
    expect(store.itens).toEqual([])
    expect(store.loading).toBe(false)
  })

  it('uma nova carga bem-sucedida limpa o erro', async () => {
    servico.listar.mockRejectedValueOnce(new Error('falhou'))
    const store = useSaidaStore()
    await store.carregar()

    await store.carregar()

    expect(store.erro).toBeNull()
    expect(store.itens).toEqual(ITENS)
  })
})

describe('criar', () => {
  it('sucesso: retorna true, volta à página 1 e recarrega', async () => {
    const store = useSaidaStore()
    store.page = 3

    const ok = await store.criar(PAYLOAD)

    expect(ok).toBe(true)
    expect(servico.criar).toHaveBeenCalledWith(PAYLOAD)
    expect(ultimoFiltro().page).toBe(1)
    expect(store.page).toBe(1)
    expect(store.itens).toEqual(ITENS)
    expect(store.salvando).toBe(false)
  })

  it('falha: retorna false, preenche erro, não recarrega e desliga salvando', async () => {
    servico.criar.mockRejectedValue(new Error('Valor inválido'))
    const store = useSaidaStore()

    const ok = await store.criar(PAYLOAD)

    expect(ok).toBe(false)
    expect(store.erro).toBe('Valor inválido')
    expect(servico.listar).not.toHaveBeenCalled()
    expect(store.salvando).toBe(false)
  })

  it('falha sem mensagem aproveitável usa o texto padrão', async () => {
    servico.criar.mockRejectedValue('quebrou')
    const store = useSaidaStore()

    await store.criar(PAYLOAD)

    expect(store.erro).toBe('Não foi possível salvar a saída.')
  })

  it('mantém salvando verdadeiro enquanto grava', async () => {
    let concluir!: () => void
    servico.criar.mockReturnValue(new Promise((resolve) => (concluir = () => resolve(saida('novo')))))
    const store = useSaidaStore()

    const promessa = store.criar(PAYLOAD)
    expect(store.salvando).toBe(true)
    concluir()
    await promessa

    expect(store.salvando).toBe(false)
  })
})

describe('atualizar', () => {
  it('sucesso: retorna true e recarrega mantendo a página', async () => {
    const store = useSaidaStore()
    store.page = 2

    const ok = await store.atualizar('s1', PAYLOAD)

    expect(ok).toBe(true)
    expect(servico.atualizar).toHaveBeenCalledWith('s1', PAYLOAD)
    expect(ultimoFiltro().page).toBe(2)
    expect(store.salvando).toBe(false)
  })

  it('falha: retorna false, preenche erro, não recarrega e desliga salvando', async () => {
    servico.atualizar.mockRejectedValue(new Error('Não encontrada'))
    const store = useSaidaStore()

    const ok = await store.atualizar('s1', PAYLOAD)

    expect(ok).toBe(false)
    expect(store.erro).toBe('Não encontrada')
    expect(servico.listar).not.toHaveBeenCalled()
    expect(store.salvando).toBe(false)
  })

  it('falha sem mensagem aproveitável usa o texto padrão', async () => {
    servico.atualizar.mockRejectedValue(undefined)
    const store = useSaidaStore()

    await store.atualizar('s1', PAYLOAD)

    expect(store.erro).toBe('Não foi possível atualizar a saída.')
  })
})

describe('remover', () => {
  it('sucesso: retorna true e recarrega', async () => {
    const store = useSaidaStore()
    await store.carregar()
    servico.listar.mockClear()

    const ok = await store.remover('s1')

    expect(ok).toBe(true)
    expect(servico.remover).toHaveBeenCalledWith('s1')
    expect(servico.listar).toHaveBeenCalledTimes(1)
    expect(store.salvando).toBe(false)
  })

  it('remover o único item de uma página > 1 recua uma página', async () => {
    const store = useSaidaStore()
    store.itens = [saida('s1')]
    store.page = 3

    await store.remover('s1')

    expect(ultimoFiltro().page).toBe(2)
    expect(store.page).toBe(2)
  })

  it('não recua na página 1, mesmo removendo o único item', async () => {
    const store = useSaidaStore()
    store.itens = [saida('s1')]
    store.page = 1

    await store.remover('s1')

    expect(ultimoFiltro().page).toBe(1)
  })

  it('não recua quando a página ainda tem outros itens', async () => {
    const store = useSaidaStore()
    store.itens = [saida('s1'), saida('s2')]
    store.page = 3

    await store.remover('s1')

    expect(ultimoFiltro().page).toBe(3)
  })

  it('falha: retorna false, preenche erro, não recarrega e desliga salvando', async () => {
    servico.remover.mockRejectedValue(new Error('Sem permissão'))
    const store = useSaidaStore()
    store.itens = [saida('s1')]
    store.page = 3

    const ok = await store.remover('s1')

    expect(ok).toBe(false)
    expect(store.erro).toBe('Sem permissão')
    expect(servico.listar).not.toHaveBeenCalled()
    expect(store.salvando).toBe(false)
  })

  it('falha sem mensagem aproveitável usa o texto padrão', async () => {
    servico.remover.mockRejectedValue(null)
    const store = useSaidaStore()

    await store.remover('s1')

    expect(store.erro).toBe('Não foi possível excluir a saída.')
  })
})

describe('alternarStatus', () => {
  it('PAGO vira PENDENTE', async () => {
    const store = useSaidaStore()

    const ok = await store.alternarStatus(saida('s1', { status: 'PAGO', pagoEm: '2026-08-10' }))

    expect(ok).toBe(true)
    const [id, payload] = servico.atualizar.mock.calls[0]!
    expect(id).toBe('s1')
    expect(payload.status).toBe('PENDENTE')
  })

  it('PENDENTE vira PAGO', async () => {
    const store = useSaidaStore()

    await store.alternarStatus(saida('s1', { status: 'PENDENTE' }))

    expect(servico.atualizar.mock.calls[0]![1].status).toBe('PAGO')
  })

  it('envia payload limpo: sem id, criadoEm e atualizadoEm, com os demais campos', async () => {
    const store = useSaidaStore()
    const original = saida('s1', { status: 'PAGO', valor: 321, descricao: 'Luz', recorrente: true })

    await store.alternarStatus(original)

    const payload = servico.atualizar.mock.calls[0]![1]
    expect(payload).not.toHaveProperty('id')
    expect(payload).not.toHaveProperty('criadoEm')
    expect(payload).not.toHaveProperty('atualizadoEm')
    expect(payload).toMatchObject({
      descricao: 'Luz',
      valor: 321,
      data: '2026-08-10',
      categoriaId: 'cat_1',
      tipo: 'CONTA',
      formaPagamento: 'PIX',
      recorrente: true,
    })
  })

  it('não altera o objeto original recebido', async () => {
    const store = useSaidaStore()
    const original = saida('s1', { status: 'PAGO' })

    await store.alternarStatus(original)

    expect(original.status).toBe('PAGO')
    expect(original.id).toBe('s1')
    expect(original.criadoEm).toBe('2026-01-01T00:00:00.000Z')
  })

  it('recarrega a lista e devolve false quando a atualização falha', async () => {
    const store = useSaidaStore()
    await store.alternarStatus(saida('s1'))
    expect(servico.listar).toHaveBeenCalledTimes(1)

    servico.atualizar.mockRejectedValue(new Error('falhou'))
    const ok = await store.alternarStatus(saida('s1'))

    expect(ok).toBe(false)
    expect(store.erro).toBe('falhou')
  })
})

describe('itensOrdenados', () => {
  it('ordena por vencimento, sem vencimento por último e mais recente primeiro', async () => {
    servico.listar.mockResolvedValue({
      items: [
        saida('sem-antiga', { data: '2026-08-01' }),
        saida('tarde', { vencimento: '2026-08-25' }),
        saida('sem-recente', { data: '2026-08-20' }),
        saida('cedo', { vencimento: '2026-08-03' }),
      ],
      page: 1,
      pageSize: 8,
      total: 4,
      totalPages: 1,
    })
    const store = useSaidaStore()

    await store.carregar()

    expect(store.itensOrdenados.map((s) => s.id)).toEqual(['cedo', 'tarde', 'sem-recente', 'sem-antiga'])
  })

  it('não muta itens', async () => {
    const recebidos = [saida('b', { vencimento: '2026-08-25' }), saida('a', { vencimento: '2026-08-03' })]
    servico.listar.mockResolvedValue({ items: recebidos, page: 1, pageSize: 8, total: 2, totalPages: 1 })
    const store = useSaidaStore()
    await store.carregar()

    const ordenados = store.itensOrdenados

    expect(ordenados.map((s) => s.id)).toEqual(['a', 'b'])
    expect(store.itens.map((s) => s.id)).toEqual(['b', 'a'])
  })

  it('não depende da ordem em que os dados chegaram', async () => {
    const dados = [
      saida('x', { vencimento: '2026-08-05' }),
      saida('y', { vencimento: '2026-08-15' }),
      saida('z', { data: '2026-08-30' }),
    ]
    const store = useSaidaStore()

    store.itens = [...dados]
    const primeiraOrdem = store.itensOrdenados.map((s) => s.id)
    store.itens = [...dados].reverse()
    const segundaOrdem = store.itensOrdenados.map((s) => s.id)

    expect(primeiraOrdem).toEqual(['x', 'y', 'z'])
    expect(segundaOrdem).toEqual(primeiraOrdem)
  })
})

describe('irParaPagina', () => {
  it('respeita os limites mínimo e máximo e recarrega', async () => {
    const store = useSaidaStore()
    await store.carregar()
    expect(store.totalPages).toBe(3)
    servico.listar.mockClear()

    store.irParaPagina(0)
    await flushPromises()
    expect(ultimoFiltro().page).toBe(1)

    store.irParaPagina(99)
    await flushPromises()
    expect(ultimoFiltro().page).toBe(3)
    expect(store.page).toBe(3)

    store.irParaPagina(2)
    await flushPromises()
    expect(ultimoFiltro().page).toBe(2)
    expect(store.page).toBe(2)

    expect(servico.listar).toHaveBeenCalledTimes(3)
  })
})

describe('filtros', () => {
  it('filtrarPorCategoria volta à página 1 e recarrega', async () => {
    const store = useSaidaStore()
    store.page = 3

    store.filtrarPorCategoria('cat_7')
    await flushPromises()

    expect(store.categoriaId).toBe('cat_7')
    expect(store.page).toBe(1)
    expect(ultimoFiltro()).toMatchObject({ categoriaId: 'cat_7', page: 1 })
  })

  it('filtrarPorStatus volta à página 1 e recarrega', async () => {
    const store = useSaidaStore()
    store.page = 3

    store.filtrarPorStatus('PAGO')
    await flushPromises()

    expect(store.status).toBe('PAGO')
    expect(store.page).toBe(1)
    expect(ultimoFiltro()).toMatchObject({ status: 'PAGO', page: 1 })
  })

  it('filtrarPorStatus(null) remove o filtro de status', async () => {
    const store = useSaidaStore()
    store.status = 'PENDENTE'

    store.filtrarPorStatus(null)
    await flushPromises()

    expect(store.status).toBeNull()
    expect(ultimoFiltro().status).toBeNull()
  })

  it('buscar volta à página 1 e recarrega', async () => {
    const store = useSaidaStore()
    store.page = 3

    store.buscar('mercado')
    await flushPromises()

    expect(store.busca).toBe('mercado')
    expect(store.page).toBe(1)
    expect(ultimoFiltro()).toMatchObject({ busca: 'mercado', page: 1 })
  })

  it('limparFiltros zera categoria, status e busca, volta à página 1 e recarrega', async () => {
    const store = useSaidaStore()
    store.categoriaId = 'cat_7'
    store.status = 'PAGO'
    store.busca = 'mercado'
    store.page = 3

    store.limparFiltros()
    await flushPromises()

    expect(store.categoriaId).toBeNull()
    expect(store.status).toBeNull()
    expect(store.busca).toBe('')
    expect(store.page).toBe(1)
    expect(ultimoFiltro()).toMatchObject({ categoriaId: null, status: null, busca: '', page: 1 })
  })
})

describe('temFiltroAtivo', () => {
  it('é falso sem categoria, status e busca', () => {
    expect(useSaidaStore().temFiltroAtivo).toBe(false)
  })

  it('é verdadeiro com categoria', () => {
    const store = useSaidaStore()

    store.categoriaId = 'cat_1'

    expect(store.temFiltroAtivo).toBe(true)
  })

  it('é verdadeiro com status', () => {
    const store = useSaidaStore()

    store.status = 'PENDENTE'

    expect(store.temFiltroAtivo).toBe(true)
  })

  it('é verdadeiro com busca preenchida', () => {
    const store = useSaidaStore()

    store.busca = 'abc'

    expect(store.temFiltroAtivo).toBe(true)
  })

  it('busca só com espaços não conta como filtro', () => {
    const store = useSaidaStore()

    store.busca = '   '

    expect(store.temFiltroAtivo).toBe(false)
  })
})

describe('vazio', () => {
  it('é verdadeiro sem itens e sem loading', () => {
    expect(useSaidaStore().vazio).toBe(true)
  })

  it('é falso com itens', async () => {
    const store = useSaidaStore()

    await store.carregar()

    expect(store.vazio).toBe(false)
  })

  it('é falso enquanto carrega, mesmo sem itens', async () => {
    let concluir!: () => void
    servico.listar.mockReturnValue(
      new Promise((resolve) => {
        concluir = () => resolve({ items: [], page: 1, pageSize: 8, total: 0, totalPages: 1 })
      }),
    )
    const store = useSaidaStore()

    const promessa = store.carregar()
    expect(store.vazio).toBe(false)
    concluir()
    await promessa

    expect(store.vazio).toBe(true)
  })
})

describe('totalPeriodo / variacao', () => {
  it('são 0 sem resumo', () => {
    const store = useSaidaStore()

    expect(store.totalPeriodo).toBe(0)
    expect(store.variacao).toBe(0)
  })

  it('totalPeriodo vem do resumo', async () => {
    servico.resumo.mockResolvedValue(resumo({ total: 1234 }))
    const store = useSaidaStore()

    await store.carregar()

    expect(store.totalPeriodo).toBe(1234)
  })

  it('variacao compara com o mês anterior', async () => {
    servico.resumo.mockResolvedValue(resumo({ total: 150, totalMesAnterior: 100 }))
    const store = useSaidaStore()

    await store.carregar()

    expect(store.variacao).toBe(0.5)
  })

  it('variacao com mês anterior zerado vira 1 se houve saída e 0 se não houve', async () => {
    const store = useSaidaStore()

    servico.resumo.mockResolvedValue(resumo({ total: 150, totalMesAnterior: 0 }))
    await store.carregar()
    expect(store.variacao).toBe(1)

    servico.resumo.mockResolvedValue(resumo({ total: 0, totalMesAnterior: 0 }))
    await store.carregar()
    expect(store.variacao).toBe(0)
  })
})
